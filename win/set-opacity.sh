...
case "$SIGN" in
  ...
  "+")
    DELTA="${NEW_OPACITY_PERCENT:1}"
    NEW_OPACITY_PERCENT=$(($CURRENT_OPACITY_PERCENT + DELTA))
    ;;
...
if [ "$NEW_OPACITY_PERCENT" -lt "1" ]; then
  NEW_OPACITY_PERCENT=1
fi

if [ "$NEW_OPACITY_PERCENT" -gt "100" ]; then
  NEW_OPACITY_PERCENT=100
fi
...
echo "NEW_OPACITY_PERCENT: $NEW_OPACITY_PERCENT"
